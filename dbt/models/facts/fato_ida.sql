...
FROM calc c
JOIN {{ ref('dim_tempo') }} dt ON c.ano_mes = dt.ano_mes
JOIN {{ ref('dim_grupo_economico') }} dg ON c.grp = dg.nome_grupo
JOIN {{ ref('dim_servico') }} ds ON c.svc = ds.codigo_servico
